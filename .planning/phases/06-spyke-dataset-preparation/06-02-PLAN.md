...
<objective>
Human reviews the 18 dry-run preview crops from Plan 01 and approves/rejects/adjusts coordinates. Claude then runs final crop generation, writes caption `.txt` files for every image, and generates pre-flipped copies for symmetric-only crops (DATA-04). Updates `.gitignore` to exclude PNG images but commit captions.
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <what-built>
18 preview crop images from Plan 01, located at:
`dataset/spyke/train/10_spyke_plasma_v1/preview/`

Each PNG is a 512×512 letterboxed crop from one of these sources:
- `Spyke_Final.png`: 4 body views (front, 3q, side, back) + 4 expression faces
...
4. **Expression face crops:** Are the 4 faces (neutral, angry, battle, shocked) readable at 512×512?
...
If all 18 look good, type: "approved — all 18 look good"
If some need changes: "approved with changes: [list each crop id and what to do]"
  </how-to-verify>
  <verify>18 preview PNGs exist in `dataset/spyke/train/10_spyke_plasma_v1/preview/` and user has reviewed them</verify>
  <done>User has typed "approved" (with optional adjustment list)</done>
  <resume-signal>Type "approved" (optionally with changes list) to proceed, or describe which crops to fix</resume-signal>
</task>
...
Update coordinates or remove excluded crops from the `CROP_SOURCES` array in `pipeline/src/scripts/crop-spyke.ts`. If coordinates were adjusted, re-run `--dry-run` to confirm fix before proceeding to final output. Skip this step if user approved all 18.
...
<success_criteria>
Training dataset directory contains 15–20 original Spyke images at 512×512 plus flip copies for symmetric poses only. Every image has a valid caption file. `.gitignore` correctly excludes PNG images while caption `.txt` files remain committed. DATA-01, DATA-02, and DATA-04 are satisfied.
</success_criteria>
