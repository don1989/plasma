...
- `spyke_final_battle`:  { left: 850,  top: 1230, width: 380, height: 280  }, flip: true,  caption: "spyke_plasma_v1, closeup face, battle focus expression, white background"
  // NOTE: ~830px gap between spyke_final_battle (left:850) and spyke_final_shocked (left:1680).
  // A 5th expression panel ('calm') may exist in this space. spyke_final_calm below is SPECULATIVE —
  // coordinates must be verified by the dry-run preview. If no distinct calm panel exists at x=1260,
  // the user should exclude this crop during the Plan 02 checkpoint review.
- `spyke_final_calm`:    { left: 1260, top: 1230, width: 380, height: 280  }, flip: true,  caption: "spyke_plasma_v1, closeup face, calm expression, white background"
...
Total: 19 crop sources (18 confirmed + 1 speculative calm expression). The speculative calm crop may be excluded after dry-run review, bringing the count to 18.
...
```
Expected: Script completes without error. `dataset/spyke/train/10_spyke_plasma_v1/preview/` contains 19 PNG files (preview_spyke_final_front.png through preview_v2_back.png, including preview_spyke_final_calm.png). Script prints a summary table. No files written to the final output directory (only preview/).
  </verify>
  <done>
19 preview crop PNGs exist in `dataset/spyke/train/10_spyke_plasma_v1/preview/`. The crop script exits 0 in dry-run mode. The CROPS config table is visible at the top of the script file, with the speculative calm expression crop noted as requiring user verification.
  </done>
</task>
...
cd /Users/dondemetrius/Code/plasma
tsx pipeline/src/scripts/crop-spyke.ts --dry-run
ls dataset/spyke/train/10_spyke_plasma_v1/preview/ | wc -l  # expect 19 (or 18 if calm crop fails)
ls dataset/spyke/reg/1_anime_character/                      # expect .gitkeep
```
</verification>

<success_criteria>
`crop-spyke.ts` script exists and runs cleanly in dry-run mode. 19 preview PNGs are present in the preview directory (or 18 if spyke_final_calm fails due to no panel at that location). User can open any preview PNG and see a 512×512 letterboxed crop of the expected Spyke reference area. Directory scaffolding committed.
</success_criteria>
...
- Note whether spyke_final_calm produced a visible preview (speculative — may be blank/duplicate)
